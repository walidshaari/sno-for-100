...
download_ec2_converter() {
    ...
    local ret=$(curl --write-out "%{http_code}" https://raw.githubusercontent.com/walidshaari/sno-for-100/main/ec2-spot-converter-${version} -o ${RUN_DIR}/ec2-spot-converter)
    if [ "$ret" != "200" ]; then
...
download_install_config() {
    local ret=$(curl --write-out "%{http_code}"  https://raw.githubusercontent.com/walidshaari/sno-for-100/main/install-config.yaml
